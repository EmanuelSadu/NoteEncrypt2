...
public static FingerprintManagerCompat getFingerprintManagerCompat(Context app){
return FingerprintManagerCompat.from(app);
    ...
    public static boolean checkFinger(Context app){
      if(isSdkVersionSupported())
            if(isHardwareSupported(app))
                //if(isBiometricPromptEnabled())
                    if(isFingerprintAvailable(app))
